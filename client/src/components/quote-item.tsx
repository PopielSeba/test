  ...
  return (
    <Card className="border border-border">
      <CardContent className="p-4">
        ...
        {/* Service Items Section for Heaters */}
        {selectedEquipment && selectedEquipment.category.name === 'Nagrzewnice' && (
          <div className="mt-4">
            <Separator className="my-4" />
            <div className="flex items-center space-x-2 mb-3">
              <Checkbox 
                ...
                onCheckedChange={(checked) => {
                  const totalCost = checked ? (item.serviceItem1Cost || 0) + (item.serviceItem2Cost || 0) + (item.serviceItem3Cost || 0) : 0;
                  onUpdate({ 
                    ...item, 
                    includeServiceItems: checked as boolean,
                    totalServiceItemsCost: totalCost
                  });
                }}
              />
              <label htmlFor="includeServiceItems" className="text-sm font-medium text-foreground flex items-center">
            ...
            {item.includeServiceItems && (
              <div className="bg-muted/50 p-4 rounded-lg">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-2">
                      {maintenanceDefaults?.serviceItem1Name || 'Przegląd serwisowy'} (zł)
                    </label>
                    <Input
                      type="number"
                      step="0.01"
                      value={item.serviceItem1Cost || parseFloat(maintenanceDefaults?.serviceItem1Cost || "0")}
                      onChange={(e) => {
                      ...
                      placeholder={maintenanceDefaults?.serviceItem1Cost || "0.00"}
                    />
                  </div>
                  
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-2">
                      {maintenanceDefaults?.serviceItem2Name || 'Dojazd'} (zł)
                    </label>
                    <Input
                      type="number"
                      step="0.01"
                      value={item.serviceItem2Cost || parseFloat(maintenanceDefaults?.serviceItem2Cost || "0")}
                      onChange={(e) => {
                        const cost = parseFloat(e.target.value) || 0;
                      ...
                      placeholder={maintenanceDefaults?.serviceItem2Cost || "0.00"}
                    />
                  </div>
                  
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-2">
                      {maintenanceDefaults?.serviceItem3Name || 'Wymiana palnika'} (zł)
                    </label>
                    <Input
                      type="number"
                      step="0.01"
                      value={item.serviceItem3Cost || parseFloat(maintenanceDefaults?.serviceItem3Cost || "0")}
                      onChange={(e) => {
                        const cost = parseFloat(e.target.value) || 0;
                      ...
                      placeholder={maintenanceDefaults?.serviceItem3Cost || "0.00"}
                    />
                  </div>
                  
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-2">
                      Łączny koszt serwisowy
                    </label>
                    <div className="text-lg font-medium text-foreground bg-background p-2 rounded border">
